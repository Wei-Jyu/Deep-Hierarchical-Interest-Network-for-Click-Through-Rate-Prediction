...
def dynamic_att_rnn(cell, inputs, att_information, att_scores=None, sequence_length=None, initial_state=None,
  ...
  # By default, time_major==False and inputs and att_information are batch-major: shaped
  #   [batch, time, depth]
  # For internal calculations, we transpose to [time, batch, depth]
  flat_input = nest.flatten(inputs)
  flat_att_information = nest.flatten(att_information)  #

  if not time_major:
    # (B,T,D) => (T,B,D)
    flat_input = [ops.convert_to_tensor(input_) for input_ in flat_input]
    flat_input = tuple(_transpose_batch_time(input_) for input_ in flat_input)
    flat_att_information = [ops.convert_to_tensor(att_information_) for att_information_ in flat_att_information]
    flat_att_information = tuple(_transpose_batch_time(att_information_) for att_information_ in flat_att_information)
  ...
  with vs.variable_scope(scope or "rnn") as varscope:
    ...
    def _assert_has_shape(x, shape):
      ...
      att_information = nest.pack_sequence_as(structure=att_information, flat_sequence=flat_att_information)

      (outputs, final_state) = _dynamic_att_rnn_loop(
          cell,
          inputs,
          state,
          att_information,
...
def _dynamic_att_rnn_loop(cell,
                      inputs,
                      initial_state,
                      att_information,
                      parallel_iterations,
  ...
  att_information_ta = tuple(_create_ta("input_%d" % i, flat_att_information[i].dtype)
                               for i in range(len(flat_att_information)))

  att_information_ta = tuple(ta.unstack(att_information_)
                               for ta, att_information_ in zip(att_information_ta, flat_att_information))
  ...
  def _time_step(time, output_ta_t, state, att_scores=None):
     ...
     att_information_t = tuple(ta.read(time) for ta in att_information_ta)
     # Restore some shape information
     for input_, shape in zip(input_t, inputs_got_shape):
       input_.set_shape(shape[1:])
                                                                                                                                                                    
     input_t = nest.pack_sequence_as(structure=inputs, flat_sequence=input_t)
     att_information_t = nest.pack_sequence_as(structure=att_information, flat_sequence=att_information_t)

     if att_scores is not None:
         att_score = att_scores[:, time, :]
         call_cell = lambda: cell(input_t, state, att_information_t, att_score)
     else:
         call_cell = lambda: cell(input_t, state, att_information_t)
