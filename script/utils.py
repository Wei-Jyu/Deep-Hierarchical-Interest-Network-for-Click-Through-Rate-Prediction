...
class VecInnAttGRUCell(RNNCell):
    ...
    def __init__(self,
      ...
      super(VecInnAttGRUCell, self).__init__(_reuse=reuse)
    ...
    def __call__(self, inputs, state, att_information, att_score):
        return self.call(inputs, state, att_information, att_score)
                                                                        
    def call(self, inputs, state, att_information, att_score):
      ...
      if self._gate_linear is None:
        ...
        with vs.variable_scope("gates"):  # Reset gate and update gate.
          self._gate_linear = _Linear(
              [inputs, state, att_information],
              2 * self._num_units,
              True,
              bias_initializer=bias_ones,
              kernel_initializer=self._kernel_initializer)
                                                                                                        
      value = math_ops.sigmoid(self._gate_linear([inputs, state, att_information]))
...
def contextual_input(oringinal_embedding, window_width, num, EMBEDDING_DIM):
    #  num_rows,num_colums = oringinal_inputs.get_shape()
    for i in range(1, num+1):
        if i >= window_width:
            C = tf.slice(oringinal_embedding, [i-window_width, 0], [window_width, EMBEDDING_DIM])
        else:
            C = tf.slice(oringinal_embedding, [0, 0], [i, EMBEDDING_DIM])
            paddings = tf.constant([[window_width-i, 0], [0, 0]])
            C = tf.pad(C, paddings, "CONSTANT")
        # Trainable parameters
        W3 = tf.Variable(tf.random_normal([EMBEDDING_DIM, EMBEDDING_DIM], stddev=0.1))
        r = tf.Variable(tf.random_normal([EMBEDDING_DIM], stddev=0.1))
        W3_C = tf.tensordot(W3, tf.transpose(C), axes = [[1], [0]])
        e = tf.tensordot(r, tf.tanh(W3_C), axes=1)
        a = tf.nn.softmax(e)
        x_c = tf.tensordot(a, C, axes=1)
        if i == 1:
            contextual_embedding = x_c
        else:
            contextual_embedding = tf.concat([contextual_embedding, x_c], 0)
...
def dim_expand(org_embedding, rnn_outputs, time_major=False, scope=None):
    if time_major:
        # (T,B,D) => (B,T,D)
        rnn_outputs = tf.array_ops.transpose(rnn_outputs, [1, 0, 2])
    rnn_outputs_size = rnn_outputs.get_shape().as_list()[-1]  # D value - hidden size of the RNN layer
    org_embedding_size = org_embedding.get_shape().as_list()[-1]
    org_embedding = tf.layers.dense(org_embedding, rnn_outputs_size, activation=None)
    org_embedding = prelu(org_embedding, scope=scope)
    new_embedding = tf.tile(org_embedding, [1, tf.shape(rnn_outputs)[1]])
    new_embedding = tf.reshape(rnn_outputs, tf.shape(rnn_outputs))
    return new_embedding

def din_fcn_attention(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False, forCnn=False, scope=None):
    if isinstance(facts, tuple):
        # In case of Bi-RNN, concatenate the forward and the backward RNN outputs.
        facts = tf.concat(facts, 2)
...
def din_fcn_max_attention(query, facts, attention_size, mask, stag='null', mode='SUM', max_stag=1, time_major=False, return_alphas=False, forCnn=False):
    if isinstance(facts, tuple):
        # In case of Bi-RNN, concatenate the forward and the backward RNN outputs.
        facts = tf.concat(facts, 2)
    if len(facts.get_shape().as_list()) == 2:
